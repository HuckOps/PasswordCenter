...
func AddMachine(c *gin.Context)  {
	...
	tagSearchList := []primitive.ObjectID{tagTmp.ID}
	userTmp := user.User{}
	if err := db.DB.Collection("user").FindOne(context.TODO(),bson.M{"user": c.MustGet("username").(string),"tag": bson.M{"$all":tagSearchList}}).Decode(&userTmp); err != nil {
